...
function RiderOrder({ navigation, route, userObj }) {
    ...
    const initState = {
        ...
        isOrderClosed: false, // false
        receiptVisible: false, // false
        receiptData: null,
        finalAmountBoxValue: null
    };
    ...
    const onCompletePitstopAndGoToNextPitstop = (isLastPitstop) => {
        const proceed = () => {
            postRequest(
                '/api/Order/JoviJob',
                {
        ...
        Alert.alert(
            'Are you sure?',
            `This will ${!isLastPitstop ? "mark the current pitstop as complete" : "close this order"} by sending following information about the current pitstop:\n\n` +
    ...
    const onCompleteOrderAndGoToRiderHome = () => {
        const proceed = () => {
            postRequest(
                ...
                (error) => {
                    console.log(((error?.response) ? error.response : {}), error);
                    Alert.alert("Error occurred while Completing the order!");
                },
                true
            );
        };

        if (state.finalAmountBoxValue === null || (parseInt(state.finalAmountBoxValue) && !state.finalAmountBoxValue.match(/(\s|\.|\,|\-)/gi))) {
            Alert.alert(
                'Are you sure?',
                `This will mark this order as Complete by sending following information about the order:\n\n` +
                                    ...
                                    <View style={{ width: "100%", maxHeight: (Dimensions.get("window").height - (state.keyboardShown ? 590 + (0) - 115 : 340 - 115 + (state.orderAllocatedCollapsed ? 268 : 0))) }}>
                                        ...
                                        <View style={{ marginTop: -2 }}>
                                            <BarChart
                                                ...
                                                inactiveBarColor={state.isOrderClosed ? activeTheme.default : "#ccc"}
                                                ref={null}
                                                onDataPointClick={() => { }}
                                                style={{
                                                    paddingRight: 0,
                                            ...
                                            {(state.activePitstopIndex < state.pitstops.length - 1) ?
                                                ...
                                                (!state.isOrderClosed) ?
                                                    ...
                                                    (!state.receiptVisible) ?
                                                        ...
                                                        :
                                                        <TouchableOpacity
                                                            onPress={() => onCompleteOrderAndGoToRiderHome()}
                                                            style={{ ...styles.appButtonContainer, elevation: 0, width: Dimensions.get("window").width, position: "relative", alignSelf: "center", bottom: 0, backgroundColor: activeTheme.default }}
                                                        >
